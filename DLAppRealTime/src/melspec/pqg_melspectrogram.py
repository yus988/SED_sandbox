...
class PQGMelSpectrogram:
    def __init__(
        ...
        self.melfb = librosa.filters.mel(sr=sr, n_fft=self.n_chunk, n_mels=self.n_mels)
        self.fps = fps
        self.iter = 0
    ...
    def update(self):
        ...
        self.imageitem.setImage(
            librosa.power_to_db(np.r_[self.melspecs[pos : self.n_frames], self.melspecs[0:pos]], ref=np.max)
        )
        self.iter += 1
